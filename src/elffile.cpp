...
void ELFFILE::ELFDisplay()
{
  ...
  if((elfhdr->e_type == 0xff00) || (elfhdr->e_type == 0xffff))
  ...
  cout << endl << "Program Headers:" << endl;
  cout << "#     " << "Type             " << "Offset        " << "VirtAddr      " << "PhysAddr      "  
       << "FileSZ        " << "MemSZ         " << "FLG           " << "Align" << endl; 
  for(ii = 0; ii <  num_Phdrs; ii++)
  {
    elfPhdrptr = &elfPhdr[ii];
  ...
  if(num_Shdrs > 0)
  {
    cout << endl << "Section Headers:" << endl;
    cout << "#     " << "Name                " << "Type                  " 
         << "Addr        " << "Offset      " << "Size        " << "ES  " 
         << "FLG  " << "LNK  " << "INF  " << "AAC" << endl; 
  ...
  {
    cout << endl << "No section headers found!" << endl;
  }  
  ...
  if(get_PT_Dynamic_entrycount() > 0)
  {
    ...
    cout << endl << "Dynamic Section at 0x" << hex << elfPhdrptr->p_offset << dec << " has " 
  ...
  {
    cout << endl << "No Dynamic Sections found." << endl << endl;
  }
...
const char *ELFFILE::get_SYMTAB_SHIndex(int index)
{
  ...
  if(elfSymptr->st_shndx == SHN_ABS)
  {
    //    sprintf(tempstr2, "%08x",  elfSymptr->st_shndx);
    return("*UND*");
  }
  else
  if(elfSymptr->st_shndx == SHN_COMMON)
  {
    //    sprintf(tempstr2, "%08x",  elfSymptr->st_shndx);
    return("*COMMON*");
  }
